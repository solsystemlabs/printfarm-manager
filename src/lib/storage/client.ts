...
class MinIOStorageClient implements StorageClient {
  ...
  async put(key: string, content: string | Buffer, options?: UploadOptions): Promise<void> {
  ...
  async get(key: string): Promise<string | null> {
    try {
      console.log(`[MinIO] Downloading from bucket: ${this.bucket}, key: ${key}`)
...
export async function getStorageClient(): Promise<StorageClient> {
  ...
  if (environment === 'development') {
    ...
    console.log(`[Storage] Initializing MinIO client for environment: ${environment}`)
    console.log(`[Storage] MinIO config - bucket: ${bucket}, accessKey: ${accessKey}`)
