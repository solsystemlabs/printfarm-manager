...
describe('calculateStorageUsage', () => {
  ...
  describe('database-only mode', () => {
    it('returns zero values when database tables do not exist', async () => {
      ...
      mockPrisma.model.findMany = vi.fn().mockRejectedValue(new Error('Table does not exist'))
      mockPrisma.slice.findMany = vi.fn().mockRejectedValue(new Error('Table does not exist'))
    ...
    it('calculates storage from database when tables exist', async () => {
      ...
      const expectedPercent = ((3.5 * 1024 * 1024) / (10 * 1024 * 1024 * 1024)) * 100
